...
**NATS Agent POC** - A learning project for Rust, NATS.io messaging, and AI agents. This is a generic NATS JetStream consumer that delegates message processing to external handler programs via stdin/stdout.

**Learning Goals:** Rust async programming (Tokio), NATS.io messaging patterns (pub/sub, queues, JetStream), AI agent architecture.
...
cargo run --bin producer -- --count 10 --subject events.created
...
- Simple struct: `id`, `event_type`, `payload` (JSON Value)
- Serialized to JSON for NATS transport and handler input
...
- Handler receives JSON event via **stdin**
...
- **Processor metrics:**
  - `messages.duration` (histogram) - Processing latency with labels: `status` (success/failed), `event_type`, `error`, `subject`
- **Producer metrics:**
...
if command -v jq &> /dev/null; then
    ...
    echo "Processing event ID: $EVENT_ID, Type: $EVENT_TYPE"

    # Your business logic here
    # ...

    # Simulate variable processing time (20ms - 1 second)
    SLEEP_DURATION=$(awk -v min=0.02 -v max=1.0 'BEGIN{srand(); print min+rand()*(max-min)}')
    sleep $SLEEP_DURATION

    echo "Successfully processed event $EVENT_ID"
    exit 0
else
    echo "Received event (jq not available):"
    echo "$EVENT"
    exit 0
fi
```
...
cargo run --bin producer -- --subject events.created --count 10

# Using nats CLI
nats pub events.created '{"id":"123","event_type":"test","payload":{"message":"hello"}}'
```
