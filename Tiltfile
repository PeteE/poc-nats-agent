...
# Grafana dashboards, provisioned from JSON checked into k8s/grafana/dashboards.
# The chart's grafana-sc-dashboard sidecar watches every namespace for
# ConfigMaps labelled grafana_dashboard=1 and drops their contents into
# Grafana, so a dashboard survives a cluster rebuild instead of living only in
# Grafana's database. The JSON file stays the single source of truth -- it is
# also what you get from Grafana's "Export -> View JSON", so it can be edited
# in the UI and pasted back.
def grafana_dashboard(name, path):
    body = str(read_file(path))
    # Indent under a YAML block scalar.
    indented = '\n'.join(['    ' + line for line in body.split('\n')])
    k8s_yaml(blob('\n'.join([
        'apiVersion: v1',
        'kind: ConfigMap',
        'metadata:',
        '  name: grafana-dashboard-%s' % name,
        '  namespace: monitoring',
        '  labels:',
        '    grafana_dashboard: "1"',
        'data:',
...
grafana_dashboard('workflow-metrics', 'k8s/grafana/dashboards/workflow-metrics.json')

# Grafana UI + its dashboard ConfigMap, as one resource.
#
# The Grafana pod belongs to the kube-prometheus-stack Helm release, which is a
# single Tilt resource covering many pods (prometheus, operator, grafana, ...).
# Hanging port_forwards off that release would let Tilt pick whichever pod it
# liked -- the same trap that sent the NATS 4222 forward to nats-box. So anchor
# on the dashboard ConfigMap and select the Grafana pod explicitly.
#
# Credentials: kubectl get secret -n monitoring kube-prometheus-stack-grafana \
#   -o jsonpath='{.data.admin-password}' | base64 -d      (user: admin)
k8s_resource(
    new_name='grafana',
    objects=['grafana-dashboard-workflow-metrics:configmap'],
    extra_pod_selectors=[{
        'app.kubernetes.io/name': 'grafana',
        'app.kubernetes.io/instance': 'kube-prometheus-stack',
    }],
    port_forwards='3000:3000',  # Grafana UI
    resource_deps=['kube-prometheus-stack'],  # needs the monitoring namespace
...
# The collector pod is created by the operator from the OpenTelemetryCollector
# CR, not applied by Tilt, so Tilt cannot discover it from the CR alone --
# extra_pod_selectors matches it by the operator's labels. Without the port
# forwards, host-run binaries (cargo run --bin producer) cannot reach the
# collector and fail their OTLP export with "tcp connect error"; only
# in-cluster workloads using the service DNS name work.
k8s_resource(
    new_name='otel-collector',
    objects=['otel-collector:opentelemetrycollector'],
    extra_pod_selectors=[{
        'app.kubernetes.io/component': 'opentelemetry-collector',
        'app.kubernetes.io/instance': 'otel-system.otel-collector',
    }],
    port_forwards='4317:4317',  # OTLP gRPC (the Rust exporter uses gRPC only)
...
helm_resource(
    ...
    flags=[
        ...
        # helm_resource makes ONE Tilt resource per release, so a chart with
        # two pods leaves Tilt guessing which to port-forward to — and it
        # picks nats-box, which accepts TCP on 4222 but speaks no NATS,
        # giving an EOF handshake. nats-box is only an in-cluster debug
        # shell; the devShell's local `nats` CLI covers that already.
...
helm_resource(
    ...
    resource_deps=['nats'],
    # Without this Tilt watches nothing in the chart, so edits to templates,
    # values.yaml, or files/*.sh (the handler mounted via ConfigMap) never
    # trigger a redeploy.
    deps=['k8s/nats-agent'],
